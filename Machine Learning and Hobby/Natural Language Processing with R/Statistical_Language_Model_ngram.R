...
books <- gutenberg_download(c(47078), meta_fields = "title")
...
books_for_tf %>% 
  group_by(author) %>% 
  slice_max(bigram_tf_idf, n = 15) %>% 
  ungroup() %>%
  mutate(word = reorder(word, tf_idf)) %>%
  ggplot(aes(bigram_tf_idf, word, fill = author)) +
...
word_cors %>%
  filter(item1 %in% c("acid", "test", "urine")) %>%
  group_by(item1) %>%
  top_n(6) %>%
  ungroup() %>%
